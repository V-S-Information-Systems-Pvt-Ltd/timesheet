...
--
-- The `role` column is kept for the transition and kept in sync by a trigger
-- so any code that still reads it stays correct (e.g. legacy display). New
-- logic must key off permission_role / hierarchy_role.
...
-- Backfill from the legacy role (kept idempotent).
...
-- Constrain the allowed values (drop-then-add keeps this idempotent).
...
-- Keep the legacy `role` derivable from the two axes on every write.
