-- db/migrations/0003_telegram_no.sql
-- Telegram bot project numbers used by the "Copy bot command" panel.
--
-- Each project may carry the numeric project code the Telegram bot expects
-- (e.g. Support -> 94, Meetings -> 141). Activity types get the same field
-- so an entry whose project has no number can fall back to its activity
-- type's number. Values are seeded from the bot's project list by matching
-- project names; admins can adjust them in the UI afterwards.
--
...
-- Ensure the Telegram bot's projects exist so the panel can generate commands.
-- Idempotent: rows whose name already exists are left untouched. Uses
-- WHERE NOT EXISTS so it does not depend on a unique constraint on name.
...
-- Fills numbers for rows that already existed before this migration ran.
