-- Pin the corrected rotate_mobile_session definition at an unambiguous
-- post-head version.
--
-- Background: 20260905000000_fix_mobile_session_rotation.sql carries the
-- aliased, qualified body, but that version is ambiguous: the same version
-- was used by different SQL on an upstream branch, so version history alone
-- cannot prove which body a linked database applied. Per the release-owner
-- policy decision recorded in docs/plans/MOBILE_SUPABASE_MIGRATION_HISTORY_AUDIT.md
-- (monotonic post-head version), this migration re-creates the function at a
-- version that sorts strictly after the current head (20260910000000).
--
-- This file is a superset of the 05000000 repair:
--   * the table is aliased as `s` and every expression-level column reference
--     is qualified, so RETURNS TABLE columns cannot shadow table columns;
--   * search_path is pinned to public, pg_temp (the 20260904000000 and
--     20260905000000 bodies pinned only `public`).
-- `create or replace` makes this safe whether the target database already has
-- the original 20260904000000 body or the corrected 20260905000000 body.
