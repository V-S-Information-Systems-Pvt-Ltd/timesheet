-- 20260928000000_fix_restore_backup_tx_telegram_cast.sql
-- Fix restore_backup_tx: the projects/activity-types inserts fed the jsonb
-- text extraction (v_elem->>'telegram_no') directly into the integer
-- telegram_no columns. PostgreSQL has no assignment cast from text to
-- integer, so every restore of a backup containing a telegram number failed
-- with 'column "telegram_no" is of type integer but expression is of type
-- text'. Explicit casts keep NULL passthrough (no telegram number) intact.
