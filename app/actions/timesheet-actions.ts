// app/actions/timesheet-actions.ts
'use server'
...
import { ActionResult, consumeWriteRateLimit, peekWriteRateLimit } from './_helpers'
...
export async function logEntry(input: {
  ...
  const rate = await peekWriteRateLimit(actor)
  ...
  if (!result.error) await consumeWriteRateLimit(actor)
...
export async function bulkUpdateTimesheets(
  ...
  if (updated > 0) await consumeWriteRateLimit(actor)
  return {
