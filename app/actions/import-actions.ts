...
export async function importTimesheets(
  ...
  if (!result.error) {
    ...
    if (audit.error) {
      logger.warn('audit log failed', { action: 'timesheets.import', error: audit.error })
    }
  }
  return {
...
> {
  ...
  if (!result.error) {
    ...
    if (audit.error) {
      logger.warn('audit log failed', { action: 'backup.restore', error: audit.error })
    }
  }
  return {
    error: result.error ?? undefined,
