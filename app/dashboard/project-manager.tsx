...
export default function ProjectManager({
  ...
  const handleClearSO = async (p: Project) => {
    const { error } = await setProjectSO(p.id, '')
    if (error) toast(error, 'error')
    else {
      onChanged()
      toast('S.O. number cleared.', 'success')
    }
  }
          ...
          {projects.map(p => (
            ...
            >
              ...
              <div className="flex shrink-0 items-center gap-1">
                <Button variant="ghost" size="sm" onClick={() => handleRename(p)} title="Rename" className="px-2 text-primary-600 hover:bg-primary-50">
                ...
                <Button variant="ghost" size="sm" onClick={() => handleDelete(p)} title="Delete" className="px-2 text-rose-600 hover:bg-rose-50">
                  <IconTrash className="h-3.5 w-3.5" />
