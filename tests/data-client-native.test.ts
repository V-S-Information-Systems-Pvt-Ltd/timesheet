...
describe('backend-neutral data client', () => {
  ...
  it('getProjects normalizes a bare { data } response to error: null', async () => {
    mockFetch.mockResolvedValue(await jsonResponse({ data: [{ id: 'p1' }] }))
    expect(await dataClient.getProjects()).toEqual({ data: [{ id: 'p1' }], error: null })
  })
  ...
  it('activity types (all vs active)', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('http://localhost/api/data/activity-types', expect.any(Object))
    await dataClient.getAllActivityTypes()
    expect(mockFetch).toHaveBeenCalledWith('http://localhost/api/data/activity-types?all=1', expect.any(Object))
  })
  ...
  it('leaves: read (with filters), insert, delete', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost/api/data/leaves?userId=u1&from=2026-01-01&to=2026-02-01',
      expect.any(Object)
    )
    await dataClient.insertLeaves([{ userId: 'u1', leaveDate: 'd', reason: 'r' }])
    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost/api/data/leaves',
      expect.objectContaining({ method: 'POST' })
    )
    await dataClient.deleteLeave('l1')
    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost/api/data/leaves?id=l1',
  ...
  it('reminders: read, insert, update, delete', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('http://localhost/api/data/reminders', expect.any(Object))
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost/api/data/reminders?id=r1',
      expect.objectContaining({ method: 'DELETE' })
    )
  })
  ...
  it('global reminders: due vs all', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('http://localhost/api/data/global-reminders', expect.any(Object))
  ...
  it('getReportTotals queries /api/data/reports with query params', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      'http://localhost/api/data/reports?project=p1&from=2026-08-01&to=2026-08-31&groupBy=user',
      expect.any(Object)
    )
