// Pure heuristic for suggesting a default hours value based on the user's
// recent weekday entries. Designed to be unit-testable without browser APIs
// or native modules; shared by web and mobile.

/** A single entry in the recent log, used by the smart-hours heuristic. */
export interface LogEntry {
  log_date: string
  hours_worked: number
}

/** Minimal structural shape the platform adapters convert from. */
...
/** Convenience adapter: extract the fields the heuristic needs from a row. */
export function timesheetToLogEntry(t: LogEntrySource): LogEntry {
